...
- 하단 저장 버튼으로 수동 저장, 변경 없으면 비활성화, 미저장 상태에서 닫기 확인
...
편집창이나 설정창이 열려 있거나 입력 요소에 포커스가 있으면 주기적인 자동 동기화를 미룹니다. 저장 버튼은 즉시 동기화하고, 편집을 마친 뒤 자동 동기화가 재개됩니다. 일기 안에서 새 추가 항목을 만들면 해당 일기에 입력란도 바로 추가합니다. Google 연결 중에는 연결 버튼이 비활성화되며 시트 찾기는 기존 인증을 재사용합니다.

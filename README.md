...
- 본문의 `https://`, `http://`, `www.` 주소는 읽기 화면에서 클릭하면 새 탭으로 열기
...
원본 사진 뷰어는 PC에서 창 모서리 크기 조절·창 확대·전체 화면을 지원합니다. `맞춤`부터 원본 `100%`까지 확대/축소 버튼 또는 휠로 조절하고, 확대 후 드래그로 이동할 수 있습니다. 모바일에서는 핀치 확대/축소와 두 손가락 이동을 지원하고 가로·세로 화면에 맞춰 표시 영역을 조정합니다. 맞춤 상태는 회전 후 다시 맞추고, 확대 중에는 가능한 배율을 유지합니다. 전체 화면 API를 사용할 수 없으면 브라우저 표시 영역 전체를 채웁니다. 뷰어를 닫으면 원본 표시용 Blob URL을 해제합니다.
...
앱 시작 및 화면 복귀 때 새 버전을 확인합니다(자동 확인 간격 최소 1분). 메인 화면·일기 작성 창에 표시되는 `업데이트 적용`을 누르면 작성 중인 내용을 기기에 저장한 뒤 새 버전으로 다시 엽니다. 저장 실패나 진행 중인 동기화가 있으면 전환하지 않습니다. 설정에서는 현재 실행 중인 버전과 `업데이트 확인` 버튼을 제공합니다.

빌드는 HTML과 서비스 워커에 같은 버전 ID를 기록하고, 캐시하지 않는 `/version.json`으로 최신 버전을 확인합니다. 새 서비스 워커의 활성화와 버전 일치를 확인한 뒤에만 적용 버튼을 표시합니다. 오프라인에서는 현재 앱을 유지하며 자동 새로고침이나 사용자 데이터 삭제를 하지 않습니다. 이 개선이 없는 구버전에서는 처음 한 번 기존 설정의 새 버전 적용 또는 앱 재실행이 필요합니다.

...
5. 처음에는 **새 My Diary 시트 만들기**를 누릅니다. 기존 `My Diary` 폴더에 스프레드시트를 만들고 `일기`, `휴지통`, `일정`, `설정` 탭을 준비합니다. 기존 파일을 삭제하지 않습니다.
...
| 설정   | 형식 식별자, 이관 완료 상태와 휴지통 보관 기간                                               |

일기당 하나의 현재 값만 유지합니다. 저장할 때 기존 `일기` 행과 연결된 일정 행을 같은 `batchUpdate`로 갱신합니다. 기존 이력 파일을 처음 연결하면 최신 저장 시각의 내용을 남기고 과거 내용은 지웁니다. 기본 Sheet1/시트1은 값이 전혀 없을 때만 삭제합니다. 과거 보관 상태는 일반 기록으로 되돌리며 새 삭제 동작부터 휴지통 기간을 계산합니다.

최초 이관, 빈 행 감지 및 행 삭제 시 데이터 사이의 빈 행을 아래에서부터 실제 삭제해 기록을 연속으로 유지합니다. 삭제한 일기 본문은 별도 `휴지통` 탭으로 이동하고 복원하면 `일기` 탭으로 돌아옵니다. 사진과 일정 연결 정보는 일기·휴지통 행의 O열에 함께 저장합니다. 편집 이력 자체는 보관하지 않습니다. 같은 일기의 양쪽 수정은 자동 덮어쓰기를 중단하고 충돌로 표시합니다. 일기를 열어 양쪽 본문을 비교하고 기기 수정본을 별도 일기로 보존할 수 있습니다. 빈 행 삭제와 일기 저장·휴지통 정리는 임시 숨김 탭 `__MyDiaryLock`을 이용해 기기 간 순차 실행합니다. 작업이 끝나면 잠금 탭을 삭제하며 중단된 작업의 잠금은 10분 뒤 복구할 수 있습니다. 잠금 탭을 직접 편집하거나 삭제하지 마세요. Sheets 자체를 직접 수정하거나 이전 버전 앱으로 쓰는 작업까지 이 잠금이 보호하지는 않습니다. 앱 사용 중 시트의 행을 수동 정렬하거나 일부 열만 이동하지 마세요.

목록은 전체 제목·날짜·미리보기만 읽습니다. 일기 날짜 기준 최근 100개 본문을 IndexedDB에 보관하고 변경된 저장 식별자의 본문만 배치로 갱신합니다. 오래된 본문은 열 때 한 행만 읽습니다. 미전송 수정·열린 일기·초안은 캐시 정리에서 보호합니다. 과거 본문 검색은 날짜 범위를 좁혀 수행하며 백업은 전체를 읽습니다. 앱 이외에서 본문 셀만 직접 변경하면 저장 식별자가 바뀌지 않아 캐시 갱신 대상이 되지 않습니다.

휴지통 기간은 설정에서 0/7/30/90/365일로 지정합니다. 기본값은 30일입니다. 다음 Google 연결 때 기간이 지난 기록을 완전 삭제하며 설정에서 즉시 정리할 수도 있습니다. 앱이 닫혀 있을 때 실행하는 예약 작업은 없습니다. 완전 삭제는 본문·일정·다른 일기가 참조하지 않는 사진 원본과 썸네일을 제거합니다. 삭제되기 전에는 휴지통에서 복원할 수 있습니다. 로그인 전 로컬 기록의 휴지통은 설정에서 직접 정리합니다.

본문은 편당 12만 자까지, 셀당 3만 자씩 네 칸으로 나누어 저장합니다. 입력은 수식이 아닌 문자열로 저장합니다. 사진·일정 연결 정보의 단일 값은 45,000자 이내입니다. 서버 저장에 실패하면 로컬 기록과 원본 사진을 유지하며 저장 완료로 표시하지 않습니다.
...
- `src/journal/sheets.js`: Sheets 스키마·행 갱신·휴지통 정리·메타데이터 이관
...
추가항목 기능은 제거했습니다. 첫 연결에서 기존 추가항목의 사진·가져오기 정보와 일정 정보를 O열로 이관하고 완료 표시를 남깁니다. 설정에 **이관 완료**가 표시된 뒤 사용자가 추가항목 워크시트를 직접 삭제할 수 있습니다. 앱은 해당 워크시트를 삭제하거나 다시 생성하지 않습니다. 이관 후에는 이전 버전 앱으로 저장하지 말고 모든 기기에서 새 버전을 사용하세요.

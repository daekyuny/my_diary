...
삭제·완전 삭제 시 행 번호가 움직여 다른 기기의 저장 위치가 바뀌지 않도록 셀 내용을 비웁니다. 따라서 과거 이력을 정리한 자리에 빈 행이 남을 수 있습니다. 편집 이력 자체는 보관하지 않습니다. 같은 일기의 동시 저장은 서버에 마지막으로 도착한 값이 반영됩니다. Sheets에는 조건부 갱신 기능이 없어 DB 수준의 동시성 보장은 없습니다. 앱 사용 중 시트의 행을 수동 정렬하거나 일부 열만 이동하지 마세요.

목록은 제목·날짜·미리보기만 읽고 본문은 필요할 때 불러옵니다. 전체 검색·백업은 데이터량에 비례해 느려질 수 있습니다.

휴지통 기간은 설정에서 0/7/30/90/365일로 지정합니다. 기본값은 30일입니다. 다음 Google 연결 때 기간이 지난 기록을 완전 삭제하며 설정에서 즉시 정리할 수도 있습니다. 앱이 닫혀 있을 때 실행하는 예약 작업은 없습니다. 완전 삭제는 본문·항목·일정·다른 일기가 참조하지 않는 사진 원본을 제거합니다. 삭제되기 전에는 휴지통에서 복원할 수 있습니다. 로그인 전 로컬 기록의 휴지통은 설정에서 직접 정리합니다.
...
사진 원본은 Google Drive의 `My Diary/attachments`에 저장하고 시트에는 파일 ID를 연결합니다. JPG·PNG·WebP·GIF, 사진당 10MB까지 지원합니다. 첨부 해제·휴지통 이동만으로는 원본을 삭제하지 않습니다.
